...
TREE_LABELS="rel v08 exp rel_x4 ride ride_l8 rot"   # exp/rel_x4/ride: experimental builds
...
defines_for() { # the flags each label in the docs was measured with
  case $1 in
    ...
    ride)       echo "-DRIDE_D=2" ;;    # M1: the cue corrects its phase instead of dying
    ride_l8)    echo "-DRIDE_D=2 -DRIDE_L=8" ;;  # M1, post-hoc: a stricter test to ride on
    rot)        echo "-DCUE_ROTATE=1" ;;  # M2: either forward master may call for a cue
    base)       ;;
    cue)        echo "-DDNAC_CUE" ;;
